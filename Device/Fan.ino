...
#define FAN_PIN 13                   // Pin controlling the fan
...
void setup() {
  ...
  digitalWrite(FAN_PIN, LOW);       // Ensure fan is off on start
  Serial.begin(9600);
...
void loop() {
  if (Serial.available()) {
    ...
    if (!error) {
      ...
      // Handle "device_update" command (turn fan on/off)
...
void registerDevice() {
  ...
  doc["device_type"] = "fan";       // Changed from "light" to "fan"
  doc["pin"] = FAN_PIN;
